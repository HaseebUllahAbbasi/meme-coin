...
// imported to get the information of the coin 
    use std::type_name::{Self, TypeName};

    // multiple imports in sui  
// dynamic_field:: The dynamic_field module allows you to create flexible data structures that can grow or shrink as needed. This is useful for managing collections of items like tokens, NFTs, or other assets.

// Tables(Key Value):: provide a way to store and manage key-value pairs, which is a fundamental data structure in many applications. They are useful for scenarios where you need to associate unique keys with specific values, such as user balances, token ownership, or configuration settings.    include support for secure access patterns, atomic operations, and efficient iteration over entries


// VcMap::  key-value store (similar to a hash map or dictionary). This allows you to map keys to values efficiently and is useful for scenarios where you need to associate unique keys with specific values.
// VecSet is typically used to create a set, which is a collection of unique items. This is useful for scenarios where you need to ensure that a collection contains no duplicate items.
    use sui::{
        sui::SUI,
        dynamic_field as df,
        table::{Self, Table},
...
// performs a multiplication followed by a division, with an upward rounding. This can be useful in financial calculations where precise rounding is necessary to avoid underestimating values. For example, calculating fees, interest, or other financial metrics often requires such operations.
    use suitears::math64::{mul_div_up, min};
    
    // invariant_
    // This function probably calculates or ensures the invariant condition of a liquidity pool. In AMMs, the invariant is a mathematical condition that maintains the relationship between the amounts of two or more assets in the pool. For instance, in a constant product market maker model (like Uniswap), the invariant condition is  x*y=k, where x and y are the reserves of two assets, and k is a constant.


// get_amount_out
    //  computes the output amount of an asset given an input amount of another asset, based on the current state of the liquidity pool. It is essential for determining how much of an asset you will receive when swapping a certain amount of another asset
    ...
    // pool key map for two coins resptively      
    ...
    public struct Config has key {
        ...
        /// overall pools will be here for the coin 
...
// global storage to create the coin pool and data to take input 
...
// creates the inital coin with given dta
...
// initializes a new liquidity pool in a decentralized exchange or similar system. 
// It checks a series of conditions and actions  then pool is set up correctly and securely.
...
/// to migrate a coin thourgh the witness to dex 
/// 
...
// checks the conditons did not get the witness name and if already migrating 
        ...
        let FunPool { id } = pool;
        // get the pool id and emit the event 
        events::migrated(
            id.to_address(), 
            type_name::get<CoinX>(),
            type_name::get<CoinY>(),
    ...
    // get  the pool address of coins if created 
...
// taking fees functions from coin owner  x and y 
...
// restrict to can't exceed the max swap fee
...
// create  a new cool using the config by the admin and inputs from admin 
